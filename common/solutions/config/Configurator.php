...
namespace common\solutions\config;
...
class Configurator implements Configurable
{
    ...
    protected function getCache()
    {
        if (is_file($this->cacheFile) && (filemtime($this->cacheFile) + $this->expire) > time()) {
            $cacheConfig = self::unserializeConfiguration(unserialize(file_get_contents($this->cacheFile)));
    ...
    protected function setCache($config)
    {
        if (!YII_DEBUG) {
            file_put_contents($this->cacheFile, serialize([
                ...
                'config' => self::serializeConfiguration($config),
            ]));
        }
    }

    /**
     * Десерализация массива с учетом замыканий
     * @param $records
     * @return mixed
     */
    public static function unserializeConfiguration($records)
    {
        foreach ($records as $key => $value) {
            if (is_array($value)) {
                $records[$key] = static::unserializeConfiguration($value);
            }

            if ($value instanceof SerializableClosure) {
                $records[$key] = $value->getClosure();
            }
        }
    ...
    /**
     * Сериализация массива с учетом замыканий
     * @param array $records
     * @return array
     */
    public static function serializeConfiguration(array $records)
    {
        foreach ($records as $key => $value) {
            if (is_array($value)) {
                $records[$key] = static::serializeConfiguration($value);
            }

            if ($value instanceof \Closure) {
                $records[$key] = new SerializableClosure($value);
            }
        }
